...
import { WIZARD_STEPS, WIZARD_STEP_LABEL, type WizardStep } from '../model/draft-types';

export type WizardStepperProps = {
  current: WizardStep;
  /** Steps the user may jump back to. Forward moves go through the gates. */
        ...
        return (
          <li key={step} className="flex items-center gap-2">
            {selectable ? (
              ...
              <span className="flex items-center gap-1.5 px-1 py-0.5" aria-current={active ? 'step' : undefined}>
                {content}
              </span>
            )}
